...
CNLHelper *CNLHelper::curHelper;
CNLHelper *CNLHelper::Instance()//单例模式
{
    if (!curHelper) {
        QMutex mutex;//互斥量
        QMutexLocker locker(&mutex);
        if (!curHelper) {//如果为空则new一个对象
            curHelper = new CNLHelper;
        }
    }
    return curHelper;
}

CNLHelper::CNLHelper(QObject *parent) : QObject(parent)
{
    //从配置文件读取拼音数组
    QFile file(":/cnl.txt");
